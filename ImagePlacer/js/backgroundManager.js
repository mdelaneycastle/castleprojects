const BackgroundManager = {
    ...
    largeOrientation: 'wide', // 'wide' or 'tall'
    backgroundImages: {},
    onBackgroundChange: null,

    async preloadBackgrounds() {
        const categories = ['small', 'medium', 'large_wide', 'large_tall'];
    ...
    getCategoryLabel(category) {
        const labels = {
            ...
            large: 'Large (> 90cm)',
            large_wide: 'Large Wide (> 90cm)',
            large_tall: 'Large Tall (> 90cm)'
        };
        return labels[category] || category;
    },

    setBackground(category) {
        // Handle large category with orientation
        let actualCategory = category;
        if (category === 'large') {
            actualCategory = 'large_' + this.largeOrientation;
        }

        if (actualCategory === this.currentCategory) {
            return false; // No change
        }

        if (!this.backgroundImages[actualCategory]) {
            console.warn(`Background not loaded for category: ${actualCategory}`);
            return false;
        }
        ...
        if (this.onBackgroundChange) {
            this.onBackgroundChange(actualCategory, this.backgroundImages[actualCategory]);
        }

        return true;
    },

    setLargeOrientation(orientation) {
        this.largeOrientation = orientation;
        // If currently showing large, switch to new orientation
        if (this.currentCategory && this.currentCategory.startsWith('large_')) {
            this.currentCategory = null; // Force refresh
            this.setBackground('large');
        }
    },

    isLargeCategory() {
        return this.currentCategory && this.currentCategory.startsWith('large_');
    },
